...
export default function HubSimulador() {
    ...
    comexstat
      ...
      .then((data: any) => {
        ...
        const filtered = list.filter(
          (c: CountryOption) => c.code && c.name && c.code !== "097"
        );
  ...
  const handleNcmQueryChange = useCallback(
    (val: string) => {
      setNcmQuery(val);
      if (
        selectedNcm &&
        val !== `${selectedNcm.code} — ${selectedNcm.description}`
      ) {
        setSelectedNcm(null);
        setResult(null);
      }
  ...
  const handleSimulate = useCallback(async () => {
    ...
    try {
      ...
      if (!hasTariff && hsPrefix.length >= 6) {
        const fallbackResult = await lookupCTSTariff(
          ctsCountry,
          hsPrefix.substring(0, 4)
        );
      ...
      setSimError(err?.message || "Erro ao executar simulação. Tente novamente.");
    } finally {
      setSimulating(false);
    }
            ...
            <div className="space-y-2">
              ...
              >
                ...
                >
                  <SelectValue
                    placeholder={
                      loadingCountries ? "Carregando países..." : "Selecionar país..."
                    }
                  />
                </SelectTrigger>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <ResultCard
              ...
              value={formatCurrency(
                result.annualized ? result.estimatedAnnualFob : result.estimatedMonthlyFob
              )}
            ...
            <ResultCard
              ...
              value={formatCurrency(
                result.annualized ? result.totalImporterCostAnnual : result.totalImporterCostMonthly
              )}
              sub={
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardContent className="p-5">
              <h3 className="text-xs font-bold text-slate-600 uppercase tracking-wide mb-4">
                Composição de Custos — Base {result.annualized ? "Anual" : "Mensal"}
              </h3>
              <div className="space-y-3">
                ...
                {result.hasTariff && (
                  <div className="flex items-center justify-between py-2 border-b border-slate-100">
                    ...
                    <span className="text-sm font-bold text-slate-800">
                      {formatCurrency(
                        result.annualized ? result.tariffCostAnnual : result.tariffCostMonthly
                      )}
                    </span>
                  </div>
                )}
                {result.exportTaxRate > 0 && (
                  <div className="flex items-center justify-between py-2 border-b border-slate-100">
                    ...
                    <span className="text-sm font-bold text-slate-800">
                      {formatCurrency(
                        result.annualized ? result.exportTaxMonthly * 12 : result.exportTaxMonthly
                      )}
                    </span>
                  </div>
                )}
      ...
      {!result && !simulating && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-slate-500 max-w-md">
            Preencha o formulário acima com o NCM do produto, país de destino e volume mensal para
            calcular valores FOB estimados, tarifas de importação no destino e custos totais.
          </p>
