-- ═══════════════════════════════════════════════════════════
-- FIX COMPLETO: Criar tabelas, colunas, funções e permissões
-- ═══════════════════════════════════════════════════════════

-- 1. Garantir que a tabela profiles existe com colunas mínimas
...
-- 2. Criar tabela user_usage (sistema de percentagem)
...
BEGIN
    -- Política SELECT
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'user_usage' AND policyname = 'Users can view own usage'
    ) THEN
    ...
    -- Política UPDATE
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'user_usage' AND policyname = 'Users can update own usage'
    ) THEN
        CREATE POLICY "Users can update own usage" ON public.user_usage
            FOR UPDATE USING (auth.uid() = user_id);
    END IF;

    -- Política ALL para service_role (admin/backend)
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'user_usage' AND policyname = 'Service role manages usage'
    ) THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'profiles' AND policyname = 'Service role manages profiles'
    ) THEN
...
-- 3. Função para atualizar plano de qualquer usuário (admin)
...
-- 4. View admin para listar todos os usuários e planos
...
-- 5. Atualizar todos os plan_type nulos
...
-- 6. Criar user_usage para lacupulaurubici@gmail.com (professional)
DO $$
DECLARE
    target_id UUID;
BEGIN
...
-- 8. Garantir que auth.users seja acessível (para a view admin_user_plans)
